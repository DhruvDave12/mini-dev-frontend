...
export const ChatProvider = ({ children }: IChatProviderProps) => {
    ...
    const queryMiniDev = async (message: string) => {
        setPreviousChats((prev) => [...prev, {message, isUser: true}]);
        setQueryLoading(true);
        setLoading(true);
        try {
            const token = localStorage.getItem("@miniDev/token");
            const res = await axiosInstance.post("/data-ingestor/query", {
                query: message,
            }, {
                headers: {
                    Authorization: `Bearer ${token}`
            ...
            if(res.status === 200 || res.status === 201) {
                ...
                setPreviousChats((prev) => [...prev, message]);
            }
        } catch (error) {
    ...
    return (
        <ChatContext.Provider value={{ previousChats, getPreviousChats: getAllChats, queryMiniDev, history, loading, queryLoading}}>
            {children}
