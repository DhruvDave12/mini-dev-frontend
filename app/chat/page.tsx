...
const Chat = ({ auth }: IChatProps) => {
  const { user } = auth;
  const { history, previousChats, queryMiniDev, loading, queryLoading } =
  ...
  console.log("PREVIOUS LENGTH: ", previousChats.length);
  return !loading || (loading && queryLoading) ? (
    <div style={{ height: "100vh", width: "100%" }}>
      <div className="grid lg:grid-cols-5" style={{ height: "100vh" }}>
        <Sidebar
          ...
          onClickMessage={(msg: string) => {
            setQuery(msg);
          }}
        />
        <div
          className={`col-span-3 lg:col-span-4 lg:border-l grid grid-cols-1 grid-rows-${
            previousChats.length > 0 ? "2" : "4"
          } lg:grid-rows-3`}
          style={{
